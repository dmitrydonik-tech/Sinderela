...
## Иконки
Иконки — отдельные PNG в `assets/icons/`, подключаются `?v=<ICONVER>`. Пайплайн обработки эскиза: to_navy → trim → 150px в 176×176 → центр по массе. Толщину контура НЕ раздувать постобработкой — просить жирный эскиз.
...
Слой самообновления цен в `index.html` использует `PRICEMAP` (соответствие позиций прайса карточкам каталога). Пересборка карты:
...
- Итоговый hero — ОДИН файл `assets/hero-combined.mp4`: ателье → кросс-фейд → showreel, **старт и конец — чистые кадры (без наложений), петля встык**, чтобы на стыке не оставалось остаточных следов кросс-фейда. Рецепт сборки combined + сжатие crf33 — в истории коммитов. Клипы генерируются в Veo / Google Flow.
